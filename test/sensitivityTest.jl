...
if HO == false
	@test maximum(abs.((T_exact[:] - T))) < 0.05
else
	@test maximum(abs.((T_exact[:] - T))) < 0.002
end
...
v = rand(Float64,prod(n));
Jv = zeros(prod(n));
getSensMatVecEik(v,Jv,pEik);
Z = rand(Float64,size(Jv));
JtZ = rand(prod(n));
...
@test abs(dot(JtZ[:],v) - dot(Jv[:],Z[:])) <= 1e-8
return;
end
